...
def image_to_outer_curve_arr(image_path, simplicity, min_length, threshold=0.1, pixels_per_unit=100.0):
    ...
    raw_contours = [c for c in raw_contours if contour_length(c) >= min_length]
    ...
    for contour in raw_contours:
        ...
        if len(approx) < 3 or np.any(np.isnan(approx)):
            continue
        ...
        spline.bezier_points.add(len(approx) - 1)
    ...
    for curve in curves:
        smooth_bezier_curve(curve)
    return curves

def smooth_bezier_curve(curve_obj):
    # Ensure it's active and in object mode
    ...
    # Select all points and apply smoothing
